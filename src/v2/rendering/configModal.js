...
function updateSpeciesPreview() {
    ...
    if (sp.languages?.length || sp.languageChoiceCount) {
        let langText = sp.languages?.length ? sp.languages.join(', ') : '';
        if (sp.languageChoiceCount > 0) {
            const choiceNote = `+ choose ${sp.languageChoiceCount}`;
            langText = langText ? `${langText} ${choiceNote}` : choiceNote;
        }
        parts.push(`<b>Languages:</b> ${langText}`);
    }
    if (sp.traits?.length) {
...
function populateProficiencies() {
    ...
    // Languages — fixed display + optional selection checkboxes
    const langList = document.getElementById('dnd-v1-language-list');
    const langChoicesContainer = document.getElementById('dnd-v1-language-choices');
    if (langList) {
        const fixedLangs = wizState.speciesLanguages || [];
        const chosenLangs = wizState.languageChoices || [];
        const allLangs = [...new Set([...fixedLangs, ...chosenLangs])];
        langList.textContent = allLangs.length > 0 ? allLangs.join(', ') : 'Common';
    }

    if (langChoicesContainer) {
        const choiceCount = wizState.speciesLanguageChoiceCount || 0;
        if (choiceCount > 0) {
            const fixedSet = new Set((wizState.speciesLanguages || []).map(l => l.toLowerCase()));
            const available = COMMON_LANGUAGES.filter(l => !fixedSet.has(l.toLowerCase()));
            const chosen = wizState.languageChoices || [];

            let html = `<span class="dnd-v1-sub-title" style="margin-top:0.3em;">Choose ${choiceCount} language${choiceCount > 1 ? 's' : ''}:</span>`;
            html += '<div class="dnd-v1-skill-checks">';
            for (const lang of available) {
                const checked = chosen.includes(lang) ? ' checked' : '';
                const sel = chosen.includes(lang) ? ' class="selected"' : '';
                html += `<label${sel}><input type="checkbox" data-v1-lang="${esc(lang)}"${checked} /> ${esc(lang)}</label>`;
            }
            html += '</div>';
            langChoicesContainer.innerHTML = html;

            langChoicesContainer.querySelectorAll('input[type="checkbox"]').forEach(cb => {
                cb.addEventListener('change', () => {
                    const allChecked = langChoicesContainer.querySelectorAll('input[data-v1-lang]:checked');
                    if (allChecked.length > choiceCount) {
                        cb.checked = false;
                        return;
                    }
                    wizState.languageChoices = Array.from(allChecked).map(c => c.dataset.v1Lang);
                    // Update the fixed display
                    if (langList) {
                        const allLangs = [...new Set([...(wizState.speciesLanguages || []), ...wizState.languageChoices])];
                        langList.textContent = allLangs.length > 0 ? allLangs.join(', ') : 'Common';
                    }
                    // Update label selection styling
                    langChoicesContainer.querySelectorAll('label').forEach(lbl => {
                        lbl.className = lbl.querySelector('input')?.checked ? 'selected' : '';
                    });
                });
            });
        } else {
            langChoicesContainer.innerHTML = '';
        }
    }
}
...
function collectFromStep(step) {
    switch (step) {
        ...
        case 'species': {
            ...
            if (val) {
                ...
                if (sp) {
                    ...
                    wizState.speciesLanguageChoiceCount = sp.languageChoiceCount || 0;
        ...
        case 'proficiencies': {
            ...
            const langChecks = document.querySelectorAll('#dnd-v1-language-choices input[data-v1-lang]:checked');
            wizState.languageChoices = Array.from(langChecks).map(c => c.dataset.v1Lang);
            break;
        }
    }
}
