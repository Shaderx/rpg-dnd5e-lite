/**
 * V2 Character System - Companion Panel
...
export function getCompanionAccess() {
    if (!characterV2) return { hasAccess: false };
 ...
 */
export function renderV2CompanionPanel() {
    ...
    const cd = characterV2.companionData || {};
    if (titleEl) {
 ...
 */
export function initV2CompanionPanel() {
    const formSelect = document.getElementById('dnd-v1-companion-form');
    if (formSelect) {
        formSelect.addEventListener('change', () => {
            ...
            if (!val) {
                characterV2.companionData = { type: null, form: null, name: characterV2.companionData?.name || '', creatureType: null };
            } else {
                const [type, form] = val.split(':');
                characterV2.companionData = {
                    ...(characterV2.companionData || {}),
                    type,
                    form,
                };
                if (type === 'primal') {
                    characterV2.companionData.creatureType = null;
                }
            }
    ...
    if (nameInput) {
        nameInput.addEventListener('input', () => {
            if (!characterV2) return;
            if (!characterV2.companionData) characterV2.companionData = { type: null, form: null, name: '', creatureType: null };
            characterV2.companionData.name = nameInput.value;
            saveCharacterV2(characterV2);
    ...
    for (const radio of ctypeRadios) {
        radio.addEventListener('change', () => {
            if (!characterV2) return;
            if (!characterV2.companionData) characterV2.companionData = { type: null, form: null, name: '', creatureType: null };
            characterV2.companionData.creatureType = radio.value;
            saveCharacterV2(characterV2);
            renderV2CompanionPanel();
        });
    }
